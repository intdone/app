...
<section class="boxes-page">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <div class="sec-title">
                    <h2>{{$reservation->car_number}} {{__('რეზერვაციის რედაქტირება')}}</h2>
                    <h4>
                        {{$reservation->car_number}} - {{$reservation->services->name}} - {{$reservation->craftsman->name}} {{$reservation->craftsman->surname}}
                    </h4>
                </div>
            </div>
        </div>
        <div class="card mt-3" >
            <div class="card-body">
                <form action="{!! action('ReservationController@update', $reservation->id) !!}" method="post" id="form-update" autocomplete="off">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <input type="text" class="form-control" value="{{$reservation->car_number}}" name="car_number" required placeholder="{{__('ავტომობილის სახ. ნომერი')}}">
                                </div>
                                    ...
                                    @if(count($carMake)>0)
                                        <select class="form-control select2-multiple" data-toggle="select2" name="car_make_id" id="car_make_id"  onchange="model()" data-width="100%"  data-placeholder="{{ __('ავტომობილის მარკა') }}">
                                            ...
                                            @foreach($carMake as $item)
                                                <option {{ $reservation->car_make_id ==$item->id?'selected':''  }} value="{{$item->id}}">{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                    @endif
                                </div>
                                    ...
                                    <select class="form-control car_model" data-toggle="select2" name="car_model_id" id="car_model_id" data-width="100%">
                                        ...
                                        @if(count($carModel)>0)
                                            @foreach($carModel as $item)
                                                <option {{ $reservation->car_model_id ==$item->id?'selected':''  }} value="{{$item->id}}">{{$item->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                                    ...
                                    @if(count($services)>0)
                                        <select class="form-control" data-toggle="select2" name="services_id" id="services_id" onchange="craftsman()"  data-width="100%">
                                            ...
                                            @foreach($services as $item)
                                                <option {{ $reservation->services_id ==$item->id?'selected':''  }} value="{{$item->id}}">{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                    @endif
                                </div>
                                <div class="col-md-12 mb-3">
                                    ...
                                    <select class="form-control" data-toggle="select2" name="craftsman_id" onchange="schedule(1)" id="craftsman_id" data-width="100%">
                                        <option value="">{{__('აირჩიეთ')}}</option>
                                        @if(count($craftsman)>0)
                                            @foreach($craftsman as $item)
                                                <option {{ $reservation->craftsman_id ==$item->id?'selected':''  }} value="{{$item->id}}">{{$item->name}} {{$item->surname}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    <label>{{__('დრო')}} <b>{{$reservation->time_start}}-{{$reservation->time_end}}</b></label>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <input type="text" class="form-control" value="{{$reservation->owner_name}}" name="owner_name" placeholder="{{__('ავტომობილის მფლობელის სახელი')}}">
                                </div>
                                <div class="col-md-12 mb-3">
                                    <label>{{__('ავტომობილის მფლობელის გვარი')}}</label>
                                    <input type="text" class="form-control" value="{{$reservation->owner_surname}}" name="owner_surname" placeholder="{{__('ავტომობილის მფლობელის გვარი')}}">
                                </div>
                                <div class="col-md-12 mb-3">
                                    <label>{{__('ავტომობილის მფლობელის ნომერი')}}</label>
                                    <input type="tel" class="form-control" value="{{$reservation->owner_phone}}" name="owner_phone" placeholder="{{__('ავტომობილის მფლობელის ნომერი')}}">
                                </div>
                                @method('PUT')
                                <div class="col-md-12 mb-3">
                                    <button type="button" onclick="update()" class="btn btn-lg float-end rounded-pill btn-primary">{{__('რედაქტირება')}}</button>
                                </div> 
                            </div>

                            <div class="row align-items-center schedule-bottom">
                                <div class="col-lg-12 text-center">
                                    <h4 id="previewFiles"></h4>
                                </div>
                            </div>
    ...
    <div class="container">
        <div class="row align-items-center schedule-bottom">
            <div class="col-lg-4">
                <a href="{{url('reservation')}}"><i class="fe-chevron-left"></i>{{ __('რეზერვაციების სია') }}</a>
            </div>
        </div>
    </div>
</section>
                    ...
                    success: function (e) {
                        if(e.length){
                            ...
                            for(var i=0; i<e.length; i++){
                                var opt = document.createElement('option');
                                opt.value = e[i]['id'];
                                opt.innerHTML = e[i]['name']+" "+e[i]['code'];
                                model_id.appendChild(opt);
                            }
                        }
                    },
